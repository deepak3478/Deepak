...
MENU = """\
============================================================
   TABLE 3 CONTROLLER - Cafe Butler Robot
============================================================
  [1] Scenario 1  Home -> Kitchen -> Table3 -> Home (No Confirmation)
  [2] Scenario 2  Home -> Kitchen/Table3 (wait confirm) -> Home (Timeout -> Home)
  [3] Scenario 3  Home -> Kitchen/Table3 (wait confirm) -> Home (Table Timeout -> Kitchen -> Home)
  [4] Scenario 4  Home -> Kitchen/Table3 (Cancel going to Table -> Kitchen -> Home)
...
class Table3Controller(Node):
    """Keyboard-driven controller node for Cafe Butler Robot."""

    def __init__(self):
        super().__init__('table3_controller')
        ...
        self.get_logger().info("[CTRL] TABLE 3 Controller ready.")
...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            ...
            if key == '1':
                print("\n[KEY-1] Dispatching Scenario 1: Home -> Kitchen -> Table3 -> Home")
                node._send_order(1, 'TABLE3')

            elif key == '2':
                print("\n[KEY-2] Dispatching Scenario 2: Home -> Kitchen/Table3 -> Home (Timeout -> Home)")
                node._send_order(2, 'TABLE3')

            elif key == '3':
                print("\n[KEY-3] Dispatching Scenario 3: Home -> Kitchen/Table3 -> Home (Table Timeout -> Kitchen -> Home)")
                node._send_order(3, 'TABLE3')

            elif key == '4':
                print("\n[KEY-4] Dispatching Scenario 4: Home -> Kitchen/Table3 -> Home (Cancel going to Table -> Kitchen -> Home)")
                node._send_order(4, 'TABLE3')
